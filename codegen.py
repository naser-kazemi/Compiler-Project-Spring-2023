...
class CodeGenerator:
    def __init__(self, scanner):
        ...
        self.id_type = "void"
        self.token = None
        self.error_logger = SemanticErrorLogger()
    ...
    def def_var(self, token):
        ...
        self.scanner.SYMBOL_TABLE["id"].append([var_id, "int", address, self.current_scope])
    ...
    def def_arr(self, token):
        ...
        self.scanner.SYMBOL_TABLE["id"].append([arr_id, "int*", address, self.current_scope])

    def get_id_type(self, token):
        self.id_type = token.token_type
    ...
    def save_opr(self, token):
        operand2, operator, operand1 = self.semantic_stack.pop(), self.semantic_stack.pop(), self.semantic_stack.pop()
        # TODO: type check
        address = self.get_temp()
        self.insert_instruction(Operation.get_operation(operator), operand1, operand2, address)
    ...
    def jpf_save(self, token):
        ...
        self.program_block[dest] = Instruction(Operation.Jpf, src, str(self.index + 1), "")
        self.semantic_stack.push(self.index)
        self.index += 1
